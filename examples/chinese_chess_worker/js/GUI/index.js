...
export default function GUI(chess) {
  ...
  const dom = {
    ...
    sideBar: getDomElement("sideBar"), // 侧边栏元素
    mans: getDomElement("mans"),
    lines: getDomElement("lines"),
    clickContainer: getDomElement("clickContainer"),
  };
  ...
  function createChessMan() {
    ...
    for (let y = 0; y < 10; y++) {
      for (let x = 0; x < 9; x++) {
        ...
        if (man) {
          const key = man.key
          const domx = new DOMMan(key, chess, dom.mans, skin)
          domx.move(x, y);
        }
      }
    }
  ...
  const handleContainerClick = (e) => {
    ...
    if (position) {
      ...
      if (man && man.my === my) {
        if (gameStatus.selected) {
          gameStatus.selected.click(x, y);
        }
        gameStatus.selected = man;
        gameStatus.selected.click(x, y);
        // audio.move.play();
      } else if (gameStatus.selected) {
        if (chess.indexOfPs(gameStatus.selected.ps, [x, y])) {
          chess.move(gameStatus.selected.poi.x, gameStatus.selected.poi.y, x, y);
          gameStatus.selected.click();
          gameStatus.selected = null;
  ...
  dom.clickContainer.onclick = handleContainerClick;
